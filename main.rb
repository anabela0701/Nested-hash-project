require 'json'
hospital_location = JSON.parse(%q|
{
  "meta" : {
    "view" : {
      "id" : "f7b6-v6v3",
...
def get_hop(arr,user_input)
  user_hospital = []
  arr["data"].each do |hospital|
    i =0
   until hospital[i] == 78
      i +=1
      if arr["data"][i][9] == user_input
        user_hospital << arr["data"][i][10]
        user_hospital << arr["data"][i][13]
      end 
    end 
  end
  puts user_hospital
...
get_hop(hospital_location,"Brooklyn")
